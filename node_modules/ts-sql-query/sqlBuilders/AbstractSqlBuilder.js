...
class AbstractSqlBuilder {
    ...
    _appendSpreadValue(value, params, columnType, typeAdapter, forceTypeCast = false) {
    ...
    _appendValue(value, params, columnType, typeAdapter, forceTypeCast = false) {
        if ((0, SqlBuilder_2.hasToSql)(value)) {
            return this._appendSql(value, params);
        }
        const adaptedValue = this._transformParamToDB(value, columnType, typeAdapter);
    ...
    _appendValueParenthesis(value, params, columnType, typeAdapter, forceTypeCast = false) {
        if (this._needParenthesis(value)) {
            return '(' + this._appendValue(value, params, columnType, typeAdapter, forceTypeCast) + ')';
        }
        return this._appendValue(value, params, columnType, typeAdapter, forceTypeCast);
    }
    _appendValueParenthesisExcluding(value, params, columnType, typeAdapter, excluding, forceTypeCast = false) {
        if (this._needParenthesisExcluding(value, excluding)) {
            return '(' + this._appendValue(value, params, columnType, typeAdapter, forceTypeCast) + ')';
        }
        return this._appendValue(value, params, columnType, typeAdapter, forceTypeCast);
    ...
    _appendConditionValue(value, params, columnType, typeAdapter, forceTypeCast = false) {
        if ((0, SqlBuilder_2.hasToSql)(value)) {
            return this._appendConditionSql(value, params);
        }
        const adaptedValue = this._transformParamToDB(value, columnType, typeAdapter);
    ...
    _appendConditionValueParenthesis(value, params, columnType, typeAdapter, forceTypeCast = false) {
        if (this._needParenthesis(value)) {
            return '(' + this._appendConditionValue(value, params, columnType, typeAdapter, forceTypeCast) + ')';
        }
        return this._appendConditionValue(value, params, columnType, typeAdapter, forceTypeCast);
    }
    _appendConditionValueParenthesisExcluding(value, params, columnType, typeAdapter, excluding, forceTypeCast = false) {
        if (this._needParenthesisExcluding(value, excluding)) {
            return '(' + this._appendConditionValue(value, params, columnType, typeAdapter, forceTypeCast) + ')';
        }
        return this._appendConditionValue(value, params, columnType, typeAdapter, forceTypeCast);
    ...
    _appendParam(value, params, columnType, typeAdapter, forceTypeCast) {
    ...
    _appendConditionParam(value, params, columnType, typeAdapter, forceTypeCast) {
        ...
        if (sets) {
            ...
            if (!columns) {
                ...
                for (let i = 0, length = insertProperties.length; i < length; i++) {
                    ...
                    if ((0, Column_2.__getColumnPrivate)(column).__isPrimaryKey) {
                        continue;
                    }
                    columns += this._appendColumnNameForUpdate(column, params);
                    columns += ' = ';
    ...
    _buildUpdate(query, params) {
        ...
        if (shape) {
            ...
            for (let i = 0, length = shapeProperties.length; i < length; i++) {
                ...
                const column = (0, Column_2.__getColumnOfObject)(shape, property);
                if (!column) {
                    // Additional property provided in the value object
        ...
        for (let i = 0, length = properties.length; i < length; i++) {
            const property = properties[i];
            const column = shape ? (0, Column_2.__getColumnOfObject)(shape, property) : (0, Column_2.__getColumnOfObject)(table, property);
            if (!column) {
                // Additional property provided in the value object
                // Skipped because it is not part of the table
                // This allows to have more complex objects used in the query
    ...
    _nullIfValue(params, valueSource, value, columnType, typeAdapter) {
    ...
    _minimumBetweenTwoValues(params, valueSource, value, columnType, typeAdapter) {
    ...
    _maximumBetweenTwoValues(params, valueSource, value, columnType, typeAdapter) {
